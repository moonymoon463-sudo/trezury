...
export const EnhancedAuth: React.FC = () => {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      // Log Google OAuth attempt
      await securityService.logSecurityEvent({
        event_type: 'google_oauth_attempt',
        severity: 'medium',
        description: 'User attempting Google OAuth sign-in',
      ...
      if (error) {
        // Log failed Google OAuth
        await securityService.logSecurityEvent({
          event_type: 'google_oauth_failed',
          severity: 'high',
          description: 'Google OAuth sign-in failed',
          metadata: { 
            provider: 'google',
            error_message: error.message 
          }
