-- Corrected: Create Secure Data Access Layer with PII Protection
-- Views inherit RLS from underlying tables, so no RLS policies needed on views
...
-- Enhance existing RLS policy on profiles table to include rate limiting
DROP POLICY IF EXISTS "Users can view own profile" ON public.profiles;
DROP POLICY IF EXISTS "Users can view own profile securely" ON public.profiles;

CREATE POLICY "Secure profile access with enhanced logging"
ON public.profiles
FOR SELECT
TO authenticated
USING (
