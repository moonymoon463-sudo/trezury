...
BEGIN
  ...
  IF quote_record.side = 'buy' THEN
    ...
    VALUES 
      (auth.uid(), 'USDC', -quote_record.input_amount, current_timestamp),
      (auth.uid(), 'GOLD', quote_record.grams, current_timestamp);
  ELSE
    -- Selling gold: subtract GOLD, add USDC  
    INSERT INTO balance_snapshots (user_id, asset, amount, snapshot_at)
    VALUES 
      (auth.uid(), 'GOLD', -quote_record.grams, current_timestamp),
      (auth.uid(), 'USDC', quote_record.output_amount, current_timestamp);
  END IF;
...
GRANT EXECUTE ON FUNCTION public.execute_transaction TO authenticated;